...
Let's try to match the countries in the subsets:
...
This output is as if I used the `semi_join` function keeping only matching values and columns from only one of the subsets. 

The `dplyr` joins can save some lines of code but I can see how `merge()` and `match()` can be handy :smile:
